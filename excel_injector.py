...
def inject_excel_fase1(template_stream, metadata, parsed_fat, parsed_poles, mode="cluster"):
    # Mematikan link eksternal perusak rumus (keep_links=False)
    ...
    # =============================================================================
    # 5. MANAJEMEN DELETION (BERDASARKAN ACUAN BOGO_REFERENCE.PY)
    # =============================================================================
    # Master sheet selalu dihapus (dibersihkan) di kedua mode
    sheets_to_remove = ["MASTER_FAT", "MASTER_POLE", "BA Splitter FAT"]
    
    # Pemindaian dinamis untuk mendeteksi sheet dengan nama belakang yang bervariasi (Line A, Line B, dsb)
    existing_sheets = wb.sheetnames
    for s_name in existing_sheets:
        name_lower = s_name.lower().strip()
        
        if mode == "cluster":
            # Memusnahkan sheet yang khusus milik Subfeeder
            ...
            # Memusnahkan sheet yang khusus milik Cluster
    ...
    # Eksekusi penghapusan menggunakan Set agar tidak ada duplikasi perintah
    for s_name in set(sheets_to_remove):
        if s_name in wb.sheetnames:
            wb.remove(wb[s_name])

    # =============================================================================
    # 6. ABSOLUTE BLUEPRINT SORTING (Penjaga Urutan)
    # =============================================================================
    SORT_ORDER_LOWER = [
