...
def inject_excel_fase1(template_stream, metadata, parsed_fat, parsed_poles, mode="cluster"):
    ...
    # ENGINE PENGHAPUS SHEET (CLUSTER VS SUBFEEDER)
    sheets_to_remove = ["MASTER_FAT", "MASTER_POLE", "BA Splitter FAT"]
    
    for s_name in wb.sheetnames:
        name_lower = s_name.lower().strip()
        if mode == "cluster":
            if name_lower.startswith("e2e opm subfeeder") or name_lower.startswith("otdr summary 1550") or name_lower.startswith("otdr summary 1310"):
                sheets_to_remove.append(s_name)
        elif mode == "subfeeder":
            if name_lower.startswith("bast key") or name_lower.startswith("otdr sumary (fdt-fat)") or name_lower.startswith("e2e opm distribution") or name_lower.startswith("e2e opm feeder"):
                sheets_to_remove.append(s_name)
